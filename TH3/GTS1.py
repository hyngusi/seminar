def read_graph(filename):
    with open(filename, "r") as file:
        ...
        num_vertices, start_vertex = map(int, first_line.split())
        graph = []

        for _ in range(num_vertices):
            row = list(map(int, file.readline().split()))
            graph.append(row)
        return num_vertices, start_vertex, graph;
...
num_vertices, start_vertex, graph = read_graph("TH3\\tsp3.txt")
tour, totalCost = greedy_tsp1(num_vertices, start_vertex, graph)

for i in range(num_vertices):
    print(f"{tour[i]}->", end='')
print(f"{tour[num_vertices]}")
print("Chi phí đường đi ngắn nhất: " + str(totalCost))